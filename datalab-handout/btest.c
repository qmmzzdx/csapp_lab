...
static int test_function(test_ptr t)
{
    ...
    for (i = 0; i < args; i++)
    {
        test_counts[i] = gen_vals(arg_test_vals[i], t->arg_ranges[i][0], t->arg_ranges[i][1], arg_test_range[i], i);
    }
    ...
    for (a1 = 0; a1 < test_counts[0]; a1++)
    {
        ...
        else
        {
            ...
            for (a2 = 0; a2 < test_counts[1]; a2++)
            {
                if (args == 2)
                {
                    errors += test_2_arg(t->solution_funct, t->test_funct, arg_test_vals[0][a1], arg_test_vals[1][a2], t->name);
                    /* Stop testing if there is an error */
                    if (errors)
                    {
                        return errors;
                ...
                else
                {
                    ...
                    {
                        errors += test_3_arg(t->solution_funct, t->test_funct, arg_test_vals[0][a1], arg_test_vals[1][a2], arg_test_vals[2][a3], t->name);
                        /* Stop testing if there is an error */
                        if (errors)
                        {
                            return errors;
