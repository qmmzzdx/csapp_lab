...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    if ((fscanf_val = fscanf(tracefile, "%d", &(trace->sugg_heapsize))) == EOF)
    {
        fprintf(stderr, "fscanf error!\n");
	exit(1);
    }
    if ((fscanf_val = fscanf(tracefile, "%d", &(trace->num_ids))) == EOF)
    {
        fprintf(stderr, "fscanf error!\n");
        exit(1);
    }
    if ((fscanf_val = fscanf(tracefile, "%d", &(trace->num_ops))) == EOF)
    {
        fprintf(stderr, "fscanf error!\n");
        exit(1);
    }
    if ((fscanf_val = fscanf(tracefile, "%d", &(trace->weight))) == EOF)
    {
        fprintf(stderr, "fscanf error!\n");
        exit(1);
    }
    ...
    while (fscanf(tracefile, "%s", type) != EOF)
    {
        ...
        {
        case 'a':
            if ((fscanf_val = fscanf(tracefile, "%u %u", &index, &size)) == EOF)
	    {
	        fprintf(stderr, "fscanf error!\n");
		exit(1);
	    }
        ...
        case 'r':
            ...
            break;
        case 'f':
	    if ((fscanf_val = fscanf(tracefile, "%ud", &index)) == EOF)
            {
                fprintf(stderr, "fscanf error!\n");
                exit(1);
            }
